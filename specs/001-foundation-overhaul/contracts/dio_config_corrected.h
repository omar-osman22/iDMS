/*
 * dio_config_corrected.h — Corrected DIO Pin Configuration Contract
 *
 * Feature: 001-foundation-overhaul (US7)
 * Date: 2026-02-21
 *
 * PURPOSE:
 *   Corrected pin direction configuration for all ATmega128 I/O ports.
 *   Cross-referenced against Hardware/Hardware_Integration.md pin
 *   assignments and actual firmware register usage.
 *
 ...
 *
 * CHANGES FROM CURRENT DIO_config.h:
 *   - Port A: All pins changed from DIO_OUTPUT to DIO_INPUT (ADC inputs)
 *   - Port B: PB0 and PB3 changed from DIO_OUTPUT to DIO_INPUT
 *   - Port C: PC0, PC2, PC3 changed from DIO_INPUT to DIO_OUTPUT
 *   - Port D: PD1 changed from DIO_INPUT to DIO_OUTPUT (UART0 TX)
 *   - Port E: ADDED (was entirely missing)
 *   - Port F: ADDED (was entirely missing)
 *   - Port G: ADDED (was entirely missing)
 *
 * UART ASSIGNMENT (per actual code, NOT per HW docs):
 *   - UART0 (PD0/PD1) → GSM (SIM800L)
 *   - UART1 (PE0/PE1) → LCD (TopWay)
 *
 * NOTE: This is a CONTRACT file. The implementation will replace
 *       MCAL/DIO/DIO_config.h in the actual codebase.
 */
